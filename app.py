...
# 2. הזרקת עיצוב קסטום (Super High-Contrast & Large Fonts)
st.markdown("""
    <style>
    ...
    /* הגדלת כותרות האינפוטים והטקסטים בלגו */
    label, p, span {
    ...
    /* טבלת סייבר מותאמת אישית עם פונט גדול וקריא */
    .terminal-table {
        width: 100%;
...
# 4. לוח בקרה אינטראקטיבי (סימולטור מותאם אישית)
...
with col_param3:
    st.write("")
    st.write("")
    st.markdown(f"<p style='color: #34d399; font-weight: 800; text-align: center; font-size: 20px; margin-top: 10px;'>📡 סימולטור אישי פעיל בזמן אמת</p>", unsafe_allow_html=True)
...
# בניית הטבלה באופן ידני ב-HTML בשביל שליטה על גודל הגופן והצבעים
table_html = """
<table class="terminal-table">
    <thead>
        <tr>
            ...
            <th title="מדד RSI מתורגם למצב פסיכולוגי: מתחת ל-30 זה מכירת יתר (הזדמנות), מעל 60 זה קניית יתר (סיכון)">🌡️ מדחום מומנטום</th>
...
for pair in asset_pairs:
    try:
        # שליפת נתונים היסטוריים (חצי שנה לאיתור שיא מדויק)
        ...
        if len(base_hist) > 14 and len(lev_hist) > 14:
            ...
            # חישוב מחירי יעד כפולים (הבסיס והממונף פי 3)
            next_tranche_num = tranches_bought + 1
            next_base_drop_target = next_tranche_num * drop_interval
            next_lev_drop_target = next_base_drop_target * 3  # הצמדה למנוף פי 3
            ...
            # חישוב RSI ותרגומו למילים ברורות
